...
import com.csme.csmeapi.api.InquireAuthUserListApi;
...
import com.csme.csmeapi.fin.services.InquireAuthUserListService;
...
@Controller
public class InquireAuthUserListApiController implements InquireAuthUserListApi {
	...
	public InquireAuthUserListApiController(ObjectMapper objectMapper, HttpServletRequest request) {
	 ...
	 */
    public ResponseEntity<XAuthUserListResponse> inquireAuthList(@ApiParam(value = "Inquire Auth User List" ,required=true )  @Valid @RequestBody XAuthUserListRequest body
...
,@ApiParam(value = "Version of API" , defaultValue="1.0.0") @RequestHeader(value="version", required=false) String version
		...
		MobileAppCommonDao<XAuthUserListResponse> commonDao = null;
		Long sequence = null;
		XAuthUserListResponse xAttachmentResponse = new XAuthUserListResponse();
		InquireAuthUserListService inquireAuthUserListService = new InquireAuthUserListService();
		...
		if (accept != null) {
			try {
				...
				if (commonDao.requestAlreadyExists(requestId.toString())) {
					...
					return new ResponseEntity<XAuthUserListResponse>(xAttachmentResponse, HttpStatus.BAD_REQUEST);
				} 
				...
				if (sequence != null && requestId != null)
					xAttachmentResponse = inquireAuthUserListService.getInquireAuthUserList(body, requestId, sequence);
				...
				return new ResponseEntity<XAuthUserListResponse>(xAttachmentResponse, HttpStatus.OK);
			} catch (Exception e) {
				...
				return new ResponseEntity<XAuthUserListResponse>(xAttachmentResponse, HttpStatus.INTERNAL_SERVER_ERROR);
			}  
		}

		// Return response entity with not implemented status if Accept header is null
		return new ResponseEntity<XAuthUserListResponse>(xAttachmentResponse, HttpStatus.NOT_IMPLEMENTED);
	}
