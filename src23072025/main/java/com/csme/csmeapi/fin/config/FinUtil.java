...
import com.cs.core.xml.object.syst.dbdict.Field;
import com.csme.csmeapi.fin.services.Root;
...
public class FinUtil {
	public static final Logger logger = LogManager.getLogger("CSMEMobile");
    ...
    public static void init() {
		try {
			...
			intPath = FinConfig.getPropName("INT_PATH");
			if(intPath == null) {
				intPath = ASPathConst.getUserDirPath() + File.pathSeparator ;
			}
			logConfFilePath = FinConfig.getPropName("LOG_PATH");
			...
			CONSTANTS = FinConfig.readFileProperties(FinConfig.getPropName("CUSTOM_CONFIG"));
			
			SECDS = FinConfig.getPropName("SEC_DS");
			logger.info("SECDS is :"+SECDS);
			if(SECDS == null) {
				SECDS = DSManager.getSecuDS();
			}
			logger.info("SECDS is :"+SECDS);
			CNTY = FinConfig.getPropName("COUNTRY");
			if(CNTY == null) {
				CNTY =stx.getCountryCode();
			}
			
			BKGRP = FinConfig.getPropName("BANK_GROUP");
			if(BKGRP == null) {
				BKGRP =stx.getBankGroup();
			}
			
			TRXDS = FinConfig.getPropName("TRX_DS");
			if(TRXDS == null) {
				TRXDS = DSManager.getTrxDS(BKGRP, CNTY);
			}
			
			TRXDBTYPE = FinConfig.getPropName("TRX_DB_TYPE");
			if(TRXDBTYPE==null)
				TRXDBTYPE = DSManager.getDBType(TRXDBTYPE);
			
			SECDBTYPE = FinConfig.getPropName("SEC_DB_TYPE");
			if(SECDBTYPE==null)
	...
	public static Document getIconDescriptionDoc() throws Exception {
		Document iconDesc = XMLManager.xmlFileToDom(FinUtil.intPath+"ModuleIconList.xml");
		return iconDesc;
	}
	...
	public static String getDBFieldDesc(String fieldName,String language) throws Exception {
	    ...
	    try {
			...
			if (StringUtil.isEmpty(desc)) {
			  ...
			  if (dbdct != null) {
			   Field field = dbdct.getFieldByName(fieldName);
			    if (field != null)
			      desc = field.getDesc(); 
			  } 
			} 
