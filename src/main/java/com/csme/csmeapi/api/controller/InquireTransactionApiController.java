...
import com.csme.csmeapi.api.InquireTransactionApi;
...
import com.csme.csmeapi.fin.services.InquireTransactionApiService;
import com.csme.csmeapi.fin.services.MobileAppCommonDao;
import com.fasterxml.jackson.databind.ObjectMapper;

import io.swagger.annotations.ApiParam;
/**
 * Controller class for handling inquiries based on transactions.
 * Implements the InquireTransactionApi interface.
 */
...
@Controller
public class InquireTransactionApiController implements InquireTransactionApi {
	...
	public InquireTransactionApiController(ObjectMapper objectMapper, HttpServletRequest request) {
	 ...
	 */
	public ResponseEntity<XInquireTransactionResponse> inquireTransaction(
			@ApiParam(value = "Inquire based on Transaction", required = true) @Valid @RequestBody XInquireTransactionRequest body,
		...
		MobileAppCommonDao<XInquireTransactionResponse> commonDao = null;
		Long sequence = null;
		XInquireTransactionResponse xInquireTransactionResponse = new XInquireTransactionResponse();
		InquireTransactionApiService inquireTransactionApiService = new InquireTransactionApiService();
		...
		if (accept != null) {
			try {
				...
				if (commonDao.requestAlreadyExists(requestId.toString())) {
					...
					return new ResponseEntity<XInquireTransactionResponse>(xInquireTransactionResponse, HttpStatus.BAD_REQUEST);
				} 

				// Save audit record and get transaction inquiry
				sequence = commonDao.saveAuditRecord(body.toString(), requestId.toString(), "InquireTransactionApi");
				this.logger.info("Saving record into Audit table and sequence no is: " + sequence);

				if (sequence != null && requestId != null)
					xInquireTransactionResponse = inquireTransactionApiService.getInquiryTransaction(body, requestId, sequence);

				// Convert response to JSON string for logging
				String jsonStr = FinUtil.getJsonString(xInquireTransactionResponse);
				this.logger.info("Converting POJO class to JSON string: " + jsonStr);

				// Log final response message
				this.logger.info("Final response message is: " + xInquireTransactionResponse.toString());
				...
				return new ResponseEntity<XInquireTransactionResponse>(xInquireTransactionResponse, HttpStatus.OK);
			} catch (Exception e) {
				...
				commonDao.updateAuditRecord(sequence, xInquireTransactionResponse.toString(), null, HttpStatus.INTERNAL_SERVER_ERROR + "");

				// Return response entity with internal server error status and error response
				return new ResponseEntity<XInquireTransactionResponse>(xInquireTransactionResponse, HttpStatus.INTERNAL_SERVER_ERROR);
			}  
		}

		// Return response entity with not implemented status if Accept header is null
		return new ResponseEntity<XInquireTransactionResponse>(xInquireTransactionResponse, HttpStatus.NOT_IMPLEMENTED);
	}
}
