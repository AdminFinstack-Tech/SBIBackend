...
import com.csme.csmeapi.api.ResetpasswordApi;
...
import com.csme.csmeapi.fin.services.ResetPasswordApiService;
...
public class ResetpasswordApiController implements ResetpasswordApi {
    ...
    /**
     * Handles the reset password API endpoint.
     * 
     * @param body    Reset password request body.
     ...
     * @return ResponseEntity<XResetPasswordResponse>
     */
    public ResponseEntity<XResetPasswordResponse> resetpassword(
            @ApiParam(value = "Forgot Password", required = true) @Valid @RequestBody XResetPasswordRequest body,
        ...
        Long sequence = null;
        XResetPasswordResponse resetPasswordResponse = new XResetPasswordResponse();
        if (accept != null) {
            try {
                ResetPasswordApiService resetPasswordApiService = new ResetPasswordApiService();
                ...
                sequence = commonDao.saveAuditRecord(body.toString(), requestId.toString(), "ResetpasswordApi");
                this.logger.info("Saving record into Audit table, sequence no is: " + sequence);
                if (sequence != null && requestId != null)
                    resetPasswordResponse = resetPasswordApiService.resetPassword(body, requestId, sequence);

                this.logger.info("Final Response message: " + resetPasswordResponse);
                String jsonStr = FinUtil.getJsonString(resetPasswordResponse);
                if (jsonStr != null)
                    resetPasswordResponse = this.objectMapper.readValue(jsonStr, XResetPasswordResponse.class);
                ...
                commonDao.updateAuditRecord(sequence, resetPasswordResponse.toString(), null, HttpStatus.INTERNAL_SERVER_ERROR.toString());
                return new ResponseEntity<>(resetPasswordResponse, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
        return new ResponseEntity<>(resetPasswordResponse, HttpStatus.NOT_IMPLEMENTED);
    }
